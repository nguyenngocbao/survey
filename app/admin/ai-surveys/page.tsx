...
export default function AdminAISurveysPage() {
  const [stats, setStats] = useState<AISurveyStats | null>(null)
  ...
  const loadData = async () => {
    try {
      // Load stats
      const statsResponse = await fetch('/api/ai-surveys/stats')
      ...
      // Load surveys
      const surveysResponse = await fetch('/api/ai-surveys?limit=50')
  ...
  const handleExport = async (format: string) => {
    try {
      const response = await fetch(`/api/admin/export?type=ai&format=${format}`)
      ...
      if (response.ok) {
        ...
        const filename = contentDisposition?.match(/filename="(.+)"/)?.[1] || `ai-surveys.${format}`
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Link href="/admin">
              <Button variant="outline" className="bg-white/50 backdrop-blur-sm border-purple-200 hover:bg-purple-50">
                ← Về trang chủ Admin
              </Button>
            </Link>
            <div>
              <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-purple-800 bg-clip-text text-transparent mb-2">
                🤖 Quản lý khảo sát học sinh về AI
              </h1>
              <p className="text-gray-600">Thống kê và quản lý các khảo sát về việc sử dụng AI trong học tập</p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card className="p-6 bg-gradient-to-br from-purple-500 to-purple-600 text-white border-0 shadow-lg">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-purple-100 text-sm">Tổng số khảo sát</p>
        ...
        <Card className="p-6 mb-8 bg-white/95 backdrop-blur-sm shadow-lg border-0 rounded-2xl">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-2xl font-bold text-gray-800">Xuất dữ liệu khảo sát học sinh</h3>
        ...
        <Card className="p-6 bg-white/95 backdrop-blur-sm shadow-lg border-0 rounded-2xl">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Danh sách khảo sát học sinh</h2>
          ...
          {surveys.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mt-2">Khảo sát sẽ xuất hiện ở đây khi học sinh hoàn thành</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {survey.personalInfo.studentId} - {survey.personalInfo.class}
                      </p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setSelectedSurvey({ id: survey._id, type: 'ai' })}
                    >
                      Xem chi tiết
                    </Button>
                  </div>
